...
type AuthStep = 'loading' | 'license_entry' | 'expired_trial' | 'login' | 'store_selection';
...
const Auth: React.FC<AuthProps> = ({ onLoginSuccess, onSuperAdminLogin, t, language, setLanguage, theme, toggleTheme }) => {
    ...
    const handleShowStoreList = async () => {
        setIsLoading(true);
        setError(null);
        try {
            const allStores = (await api.getAllStores()).sort((a,b) => a.name.localeCompare(b.name));
            setStores(allStores);
    ...
    const renderLicenseForm = () => (
        <form onSubmit={handleLicenseSubmit} className="space-y-4">
            ...
            <div className="relative flex py-2 items-center">
                ...
                <span className="flex-shrink mx-4 text-xs text-gray-500 dark:text-gray-400">{t('or')}</span>
                <div className="flex-grow border-t border-gray-300 dark:border-gray-600"></div>
            </div>

            <button type="button" onClick={handleShowStoreList} disabled={isLoading} className="w-full bg-slate-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-slate-700 disabled:bg-gray-400">
                {isLoading ? `${t('loading')}...` : t('selectStoreFromList')}
            </button>
        </form>
    );
    ...
    const renderStoreSelection = () => (
      <div>
          <h2 className="text-2xl font-bold text-slate-700 dark:text-slate-200 mb-4">{t('selectYourStore')}</h2>
          <div className="max-h-60 overflow-y-auto space-y-2 pr-2">
              {stores.map(store => (
                  <button 
                      key={store.id} 
                      onClick={() => handleStoreSelected(store)}
                      className="w-full text-left rtl:text-right flex items-center p-3 bg-gray-50 dark:bg-slate-700/50 hover:bg-teal-50 dark:hover:bg-slate-700 rounded-lg transition-colors"
                  >
                      <Logo url={store.logo} className="w-10 h-10 rounded-lg bg-gray-200 object-cover mr-3 rtl:mr-0 rtl:ml-3" />
                      <span className="font-bold text-slate-800 dark:text-slate-200">{store.name}</span>
                  </button>
              ))}
          </div>
          <button onClick={() => setStep('license_entry')} className="mt-4 text-sm text-teal-600 hover:underline">{t('backToLicenseEntry')}</button>
      </div>
  );
