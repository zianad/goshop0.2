...
const FinanceAndReports: React.FC<FinanceAndReportsProps> = ({ storeId, sales, expenses, purchases, suppliers, returns, customers, users, addExpense, updateExpense, deleteExpense, deleteReturn, deleteAllReturns, t, language, theme, onReprintInvoice }) => {
        ...
        {filteredPurchases.length > 0 ? (
          <ul className="space-y-3">
            {filteredPurchases.slice().reverse().map(p => (
              <li key={p.id} className="p-4 border dark:border-slate-700 rounded-lg">
                <div className="flex justify-between items-start flex-wrap gap-2">
                  <div>
                    <p className="font-bold text-slate-800 dark:text-slate-200">{t('reference')}: {p.reference && p.reference.startsWith('purchase_ref_') ? t(p.reference as any) : (p.reference || t('unknown'))}</p>
                    <p className="text-sm text-slate-500 dark:text-slate-400">{new Date(p.date).toLocaleString(locale)}</p>
                    <p className="text-sm text-slate-500 dark:text-slate-400">{t('suppliers')}: {suppliers.find(s => s.id === p.supplierId)?.name || 'N/A'}</p>
                  </div>
                  <div className="text-left rtl:text-right">
                    <p className="font-bold text-lg text-teal-600 dark:text-teal-400">{p.totalAmount.toFixed(2)} DH</p>
                    {p.remainingAmount > 0 && <p className="text-xs font-bold text-red-500 dark:text-red-400">{t('remainingAmountLabel')}: {p.remainingAmount.toFixed(2)} DH</p>}
                  </div>
                </div>
                 <details className="mt-2 text-sm">
                  <summary className="cursor-pointer text-teal-700 dark:text-teal-400 font-semibold hover:text-teal-800">{t('purchaseDetails')}</summary>
                  <ul className="mt-2 space-y-1 pt-2 pr-4 rtl:pl-4 rtl:pr-0 border-r-2 rtl:border-l-2 rtl:border-r-0 border-teal-200 dark:border-teal-500/30 text-slate-600 dark:text-slate-300">
                      {p.items.map(item => (
                          <li key={item.variantId} className="flex justify-between items-center">
                              ...
                              <span>{item.purchasePrice.toFixed(2)} DH</span>
                          </li>
                      ))}
                  </ul>
                </details>
          ...
          <p className="text-center text-slate-500 dark:text-slate-400 py-8">{t('noPurchasesRecorded')}</p>
        )}
      </div>
    </div>
  );
  ...
  const SellerReportsView = () => {
      ...
      const sellerFilteredSales = useMemo(() => {
          if(selectedUserId === 'all') return filteredSales;
          return filteredSales.filter(s => s.userId === selectedUserId);
      }, [selectedUserId]);

      const sellerFilteredReturns = useMemo(() => {
          if(selectedUserId === 'all') return filteredReturns;
          return filteredReturns.filter(r => r.userId === selectedUserId);
      }, [selectedUserId]);

      const totalSales = sellerFilteredSales.reduce((sum, s) => sum + s.total, 0);
      const totalReturns = sellerFilteredReturns.reduce((sum, r) => sum + r.refundAmount, 0);
      const netActivity = totalSales - totalReturns;
         ...
         <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-lg">
            <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
                <h2 className="text-2xl font-bold text-slate-700 dark:text-slate-200">{t('sellerReportsAndZReport')}</h2>
                 <div className="flex items-center gap-4">
                    <select value={selectedUserId} onChange={e => setSelectedUserId(e.target.value)} className="bg-gray-50 dark:bg-slate-700 border border-gray-300 dark:border-slate-600 text-slate-900 dark:text-slate-100 text-sm rounded-lg focus:ring-teal-500 focus:border-teal-500 block p-2.5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center mb-6">
                 <div className="bg-green-100 dark:bg-green-900/50 p-4 rounded-lg text-green-800 dark:text-green-200">
                    <p className="font-bold">{t('totalSales')}</p>
                    <p className="text-2xl font-extrabold">{totalSales.toFixed(2)} DH</p>
                    <p className="text-xs">{t('numberOfSales', {count: sellerFilteredSales.length})}</p>
                </div>
                 <div className="bg-orange-100 dark:bg-orange-900/50 p-4 rounded-lg text-orange-800 dark:text-orange-200">
                    <p className="font-bold">{t('totalReturns')}</p>
                    <p className="text-2xl font-extrabold">{totalReturns.toFixed(2)} DH</p>
                     <p className="text-xs">{t('numberOfReturns', {count: sellerFilteredReturns.length})}</p>
                </div>
                 <div className="bg-blue-100 dark:bg-blue-900/50 p-4 rounded-lg text-blue-800 dark:text-blue-200">
                    <p className="font-bold">{t('netActivity')}</p>
                    <p className="text-2xl font-extrabold">{netActivity.toFixed(2)} DH</p>
                </div>
            </div>
            
             <h3 className="font-bold text-lg text-slate-600 dark:text-slate-300 mb-2">{t('transactionList')}</h3>
             <div className="max-h-96 overflow-y-auto pr-2 space-y-3">
                {[...sellerFilteredSales, ...sellerFilteredReturns].sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(tx => {
                    const isSale = 'total' in tx;
                    return (
                       <li key={tx.id} className={`p-4 border rounded-lg list-none ${isSale ? 'bg-green-50 dark:bg-green-900/20' : 'bg-orange-50 dark:bg-orange-900/20'}`}>
                          <div className="flex justify-between items-center">
                              <div>
                                  <p className="font-bold text-slate-800 dark:text-slate-200">{isSale ? t('invoiceNumber') : t('returnId')} {tx.id.slice(-6)}</p>
                                  <p className="text-sm text-slate-500 dark:text-slate-400">{new Date(tx.date).toLocaleString(locale)}</p>
                              </div>
                              <div className="text-left rtl:text-right">
                                   <p className={`font-bold text-lg ${isSale ? 'text-green-600 dark:text-green-400' : 'text-orange-600 dark:text-orange-400'}`}>{isSale ? (tx as Sale).total.toFixed(2) : (tx as Return).refundAmount.toFixed(2)} DH</p>
                              </div>
                          </div>
                           <details className="mt-2 text-sm">
                            <summary className="cursor-pointer text-cyan-700 dark:text-cyan-400 font-semibold hover:text-cyan-800">{t('viewDetails')}</summary>
                            <ul className="mt-2 space-y-1 pt-2 text-slate-600 dark:text-slate-300">
                                {tx.items.map(item => (
                                    <li key={item.id} className="flex justify-between items-center">
  ...
  const StatCard = ({ title, value, details, onClick, colorClass, icon: Icon }: { title: string, value: string, details?: string, onClick?: () => void, colorClass: string, icon: React.FC<{ className?: string }> }) => (
    <div className={`p-4 rounded-xl shadow-lg ${colorClass} text-white ${onClick ? 'cursor-pointer hover:opacity-90 transition-opacity' : ''}`} onClick={onClick}>
  ...
  const paginatedSalesHistory = salesForHistoryView.slice((salesHistoryPage - 1) * SALES_HISTORY_PAGE_SIZE, salesHistoryPage * SALES_HISTORY_PAGE_SIZE);
  const totalSalesHistoryPages = Math.ceil(salesForHistoryView.length / SALES_HISTORY_PAGE_SIZE);

  if (view === 'expenses') return <ExpensesView />;
  if (view === 'debts') return <DebtsView />;
  if (view === 'returns') return <ReturnsView />;
  if (view === 'allPurchases') return <AllPurchasesView />;
  if (view === 'supplierDebts') return <SupplierDebtsView />;
  if (view === 'sellerReports') return <SellerReportsView />;

  return (
    <div className="space-y-6">
      {editingExpense && <EditExpenseModal expense={editingExpense} onClose={() => setEditingExpense(null)} onSave={updateExpense} t={t} />}
      {showCustomItemsModal && <CustomItemsModal onClose={() => setShowCustomItemsModal(false)} total={totalCustomItemSales} sales={customItemSales} customers={customers} onReprintInvoice={onReprintInvoice} t={t} language={language} />}
      
      <div className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-lg flex justify-between items-center flex-wrap gap-4">
          <h2 className="text-xl font-bold text-slate-700 dark:text-slate-200">{t('finance')}</h2>
          <div className="flex items-center gap-2 flex-wrap">
              <select value={dateRange} onChange={e => setDateRange(e.target.value)} className="bg-gray-50 dark:bg-slate-700 border border-gray-300 dark:border-slate-600 text-slate-900 dark:text-slate-100 text-sm rounded-lg focus:ring-teal-500 focus:border-teal-500 block p-2.5">
                  <option value="all">{t('all')}</option>
              ...
              {dateRange === 'custom' && (
                  <div className="flex items-center gap-2">
                      <input type="date" value={customRange.start} onChange={e => setCustomRange(p => ({ ...p, start: e.target.value }))} className="bg-gray-50 dark:bg-slate-700 border border-gray-300 dark:border-slate-600 text-slate-900 dark:text-slate-100 text-sm rounded-lg p-2"/>
                      <span>-</span>
                      <input type="date" value={customRange.end} onChange={e => setCustomRange(p => ({ ...p, end: e.target.value }))} className="bg-gray-50 dark:bg-slate-700 border border-gray-300 dark:border-slate-600 text-slate-900 dark:text-slate-100 text-sm rounded-lg p-2"/>
                  </div>
              )}
          </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4">
        <StatCard title={t('netRevenue')} value={`${netRevenue.toFixed(2)} DH`} details={t('totalRevenueMinusReturns')} colorClass="bg-teal-600 dark:bg-teal-700" icon={FileDownIcon}/>
        <StatCard title={t('netProfit')} value={`${netProfit.toFixed(2)} DH`} details={t('realizedProfitMinusExpenses')} colorClass="bg-green-600 dark:bg-green-700" icon={FileDownIcon}/>
        <StatCard title={t('totalExpenses')} value={`${totalExpenses.toFixed(2)} DH`} details={t('clickToViewDetails')} onClick={() => setView('expenses')} colorClass="bg-red-500 dark:bg-red-600" icon={FileDownIcon}/>
        <StatCard title={t('customerDebts')} value={`${totalCustomerDebts.toFixed(2)} DH`} details={t('clickToViewDetails')} onClick={() => setView('debts')} colorClass="bg-orange-500 dark:bg-orange-600" icon={UsersIcon}/>
        <StatCard title={t('totalSupplierDebt')} value={`${totalSupplierDebt.toFixed(2)} DH`} details={t('clickToViewDetails')} onClick={() => setView('supplierDebts')} colorClass="bg-yellow-500 dark:bg-yellow-600" icon={FileDownIcon}/>
        <StatCard title={t('customItemsDetails')} value={`${totalCustomItemSales.toFixed(2)} DH`} details={t('clickToViewDetails')} onClick={() => setShowCustomItemsModal(true)} colorClass="bg-purple-600 dark:bg-purple-700" icon={SparklesIcon} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <div className="lg:col-span-3 bg-white dark:bg-slate-800 p-6 rounded-xl shadow-lg">
            <h3 className="text-lg font-bold text-slate-700 dark:text-slate-200 mb-4">{t('salesPerformance')}</h3>
             <ResponsiveContainer width="100%" height={300}>
                <BarChart data={salesByDay} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke={chartColors.grid} />
                    <XAxis dataKey="name" tick={{ fill: chartColors.tick, fontSize: 12 }} />
                    <YAxis tick={{ fill: chartColors.tick, fontSize: 12 }} />
                    <Tooltip contentStyle={{ backgroundColor: chartColors.tooltip.backgroundColor, border: `1px solid ${chartColors.tooltip.borderColor}`, color: chartColors.tooltip.color, borderRadius: '0.5rem' }} cursor={{fill: 'rgba(113, 113, 122, 0.1)'}}/>
                    <Legend wrapperStyle={{color: chartColors.legend, fontSize: '14px'}}/>
                    <Bar dataKey={t('sales')} fill={chartColors.bar} />
                </BarChart>
            </ResponsiveContainer>
        </div>
        <div className="lg:col-span-2 bg-white dark:bg-slate-800 p-6 rounded-xl shadow-lg">
             <h3 className="text-lg font-bold text-slate-700 dark:text-slate-200 mb-4">{t('topSellingProducts')}</h3>
             <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                    <Pie data={topSellingVariants} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} label={({ name, value }) => `${name} (${value} ${t('units')})`}>
                        {topSellingVariants.map((entry, index) => <Cell key={`cell-${index}`} fill={chartColors.pie[index % chartColors.pie.length]} />)}
                    </Pie>
                    <Tooltip contentStyle={{ backgroundColor: chartColors.tooltip.backgroundColor, border: `1px solid ${chartColors.tooltip.borderColor}`, color: chartColors.tooltip.color, borderRadius: '0.5rem' }}/>
                </PieChart>
            </ResponsiveContainer>
        </div>
      </div>
       <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-lg">
        <h2 className="text-2xl font-bold text-slate-700 dark:text-slate-200 mb-4">{t('salesHistory')}</h2>
         <div className="overflow-x-auto max-h-[60vh]">
            <table className="w-full text-sm text-right rtl:text-right text-slate-500 dark:text-slate-400">
                <thead className="text-xs text-slate-600 dark:text-slate-300 uppercase bg-gray-50 dark:bg-slate-700 sticky top-0">
                    <tr>
                      ...
                      <th scope="col" className="px-6 py-3">{t('profitLabel')}</th>
                      <th scope="col" className="px-6 py-3 text-left rtl:text-right">{t('actions')}</th>
                    </tr>
                </thead>
                <tbody className="divide-y divide-gray-200 dark:divide-slate-700">
                {paginatedSalesHistory.map(sale => (
                    <tr key={sale.id} className="bg-white dark:bg-slate-800 hover:bg-gray-50 dark:hover:bg-slate-700/50">
                        <th scope="row" className="px-6 py-4 font-medium text-slate-700 dark:text-slate-100 whitespace-nowrap">{sale.id.slice(-6)}</th>
                        <td className="px-6 py-4">{new Date(sale.date).toLocaleString(locale)}</td>
                        <td className="px-6 py-4">{customers.find(c => c.id === sale.customerId)?.name || t('cashCustomer')}</td>
                        <td className="px-6 py-4 font-bold text-teal-600 dark:text-teal-400">{sale.total.toFixed(2)}</td>
                        <td className="px-6 py-4">{sale.downPayment.toFixed(2)}</td>
                        <td className={`px-6 py-4 font-bold ${sale.remainingAmount > 0 ? 'text-red-500' : ''}`}>{sale.remainingAmount.toFixed(2)}</td>
                        <td className="px-6 py-4 text-green-600 dark:text-green-400 font-semibold">{sale.profit.toFixed(2)}</td>
            ...
            {paginatedSalesHistory.length === 0 && <p className="text-center text-slate-500 dark:text-slate-400 py-8">{t('noSalesForPeriod')}</p>}
        </div>
         {totalSalesHistoryPages > 1 && (
            <div className="pt-4 flex justify-center items-center gap-4 border-t border-slate-200 dark:border-slate-700 mt-4">
                <button onClick={() => setSalesHistoryPage(p => Math.max(1, p - 1))} disabled={salesHistoryPage === 1} className="p-2 bg-gray-200 dark:bg-slate-700 rounded-lg disabled:opacity-50" aria-label={t('previous')}><ArrowLeftIcon className="w-5 h-5" /></button>
                <span className="font-semibold text-slate-600 dark:text-slate-300">{t('page')} {salesHistoryPage} / {totalSalesHistoryPages}</span>
                <button onClick={() => setSalesHistoryPage(p => Math.min(totalSalesHistoryPages, p + 1))} disabled={salesHistoryPage === totalSalesHistoryPages} className="p-2 bg-gray-200 dark:bg-slate-700 rounded-lg disabled:opacity-50" aria-label={t('next')}><ArrowRightIcon className="w-5 h-5" /></button>
            </div>
        )}
      </div>
    </div>
